...
`canonical_url` removes fragments and known tracking parameters (`utm_*`, `gclid`, `fbclid`) but retains meaningful query parameters. `content_hash` is SHA-256 of canonical URL, title, and summary, separated by newlines.
...
`ArticleDeduplicator` is pure: it returns a canonical URL and content hash, but owns no SQL. `ArticlesRepository` owns parameterized SQL: it finds an article by canonical URL first, then content hash, inserts new articles, leaves editorial fields unchanged on duplicates, updates `last_seen_at`, and upserts the source association. `ArticlePersistenceService` returns `inserted` or `duplicate` for one preview.
...
3. If not found, match `articles.content_hash` so syndicated URLs can merge.
...
Unique-conflict races are treated as duplicates: the repository re-reads the matching article before upserting provenance.
...
- produces the same hash for equivalent normalized input;
- produces a different hash when canonical content changes.

### Repository tests

- inserts an article when neither unique key matches;
- treats matching canonical URL as duplicate;
- treats matching content hash as duplicate;
...
4. Different URLs with the same canonical content deduplicate.
